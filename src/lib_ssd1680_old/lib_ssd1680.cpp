...
// Partial LUT — from the original WeActStudio library for this display
static const uint8_t ssd1680_lut_partial[] = {
...
{
    int tout = 6000; // ~6s max (each vTaskDelay(1) ≈ 1ms at portTICK_PERIOD_MS=1)
    vTaskDelay(1);
...
static void ssd1680_write(ssd1680_t *disp, ssd1680_regmap_t cmd, void *data, size_t data_size)
{
    ...
    {
        // CS автоматически вернется в HIGH после передачи
    	return;
    }
...
{
    uint8_t wnd[2];
    wnd[0] = (uint8_t)(x_start & 0x1F);
    wnd[1] = (uint8_t)(x_stop & 0x1F);
...
{
    uint8_t wnd[4];
    wnd[0] = (uint8_t)(y_start & 0xFF);
    wnd[1] = (uint8_t)((y_start >> 8) & 0x01);
    wnd[2] = (uint8_t)(y_stop & 0xFF);
    wnd[3] = (uint8_t)((y_stop >> 8) & 0x01);
...
{
    disp->pos_x = (uint8_t)(x_pos & 0x1F);
    disp->pos_y = (uint16_t)(y_pos & 0x1FF);
...
static void ssd1680_setup_gate_driver(ssd1680_t *disp)
{
    ...
    gate.mux = (uint16_t)(disp->rows_cnt - 1);
...
static void ssd1680_init_sequence(ssd1680_t *disp)
{
    ...
    ssd1680_display_mode(disp, SSD1680_TEMPLUTDISP1); //WAS_POWERON_SEQ, SSD1680_TEMPLUTDISP2, SSD1680_TEMPLUTDISP1
}
...
{
    ssd1680_t *disp = (ssd1680_t *)malloc(sizeof(ssd1680_t));
    if (disp == NULL)
        return NULL;
    ...
    disp->framebuffer_bw = (uint8_t *)heap_caps_malloc(disp->framebuffer_size, MALLOC_CAP_DMA);
    if (disp->framebuffer_bw == NULL) {
        free(disp);
        return NULL;
    }

    disp->framebuffer_red = (uint8_t *)heap_caps_malloc(disp->framebuffer_size, MALLOC_CAP_DMA);
    ...
    // CS НЕ настраиваем как GPIO - его настроит SPI драйвер автоматически
    // через spics_io_num в devcfg
    gpio_set_level(disp->pinmap.reset, 1);
    gpio_set_level(disp->pinmap.dc, 1);
...
{
	ssd1680_t * disp = ssd1680_init(spi_host, pinmap, res_x, res_y, orientation);

	if (disp)
	{
		ssd1680_partial_init(disp);
	}
...
void ssd1680_partial_init(ssd1680_t *disp)
{
    ...
    // иначе контроллер продолжит использовать LUT из OTP.
    ssd1680_sw_reset(disp);

    // Загружаем LUT-таблицу для partial refresh в регистр контроллера.
...
void ssd1680_refresh(ssd1680_t *disp, uint8_t update_mode)
{
#ifdef DEBUG
	int64_t t = get_time();
#endif
